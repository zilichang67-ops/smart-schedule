...
import { Zap, LogOut, Trash2, Moon, Settings, CheckSquare, X, ChevronLeft, ChevronRight, Eye, EyeOff } from "lucide-react";
...
export function Header({
  ...
  const viewLabel = view === "week" ? t.week : view === "day" ? t.day : t.month;
  ...
  return (
    <header className="flex items-center justify-between border-b border-border/50 px-4 py-3 bg-card/50 backdrop-blur-sm">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-lg font-semibold tracking-tight hidden sm:block">{t.smartSchedule}</h1>
      </div>

      {bulkMode && bulkCount > 0 && (
        <div className="flex items-center gap-2 bg-destructive/10 rounded-lg px-3 py-1.5">
          <span className="text-sm font-medium">{bulkCount} {t.selected}</span>
      ...
      <div className="flex items-center gap-1">
        ...
        <span className="text-sm text-muted-foreground hidden sm:inline truncate max-w-[100px]">{user.email}</span>
